...
@interface SKMonitoring () <UIAlertViewDelegate> {
    NSUInteger _flushInterval;
}

// re-declare internally as readwrite
@property(atomic,retain)    SKMonitoringIdentity *people;
...
@interface SKMonitoringIdentity ()

@property(nonatomic,assign) SKMonitoring *mixpanel;
...
- (id)initWithMixpanel:(SKMonitoring *)mixpanel;

@end

@implementation SKMonitoring

static void MixpanelReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info)
{
    if (info != NULL && [(NSObject*)info isKindOfClass:[SKMonitoring class]]) {
        @autoreleasepool {
            SKMonitoring *mixpanel = (SKMonitoring *)info;
...
static SKMonitoring *sharedInstance = nil;

+ (SKMonitoring *)sharedInstanceWithToken:(NSString *)apiToken
{
...
//        NSLog(@"%@ warning empty api token", self);
    ...
    if (self = [self init]) {
        self.people = [[[SKMonitoringIdentity alloc] initWithMixpanel:self] autorelease];
...
- (NSString *)encodeAPIData:(NSArray *)array
{
    ...
    if (data) {
        b64String = [SKMPBase64 mp_base64EncodedStringWithData:data];
...
//        NSLog(@"%@ mixpanel track called with empty event parameter. using 'mp_event'", self);
    ...
    [SKMonitoring assertPropertyTypes:properties];
...
@implementation SKMonitoringIdentity
